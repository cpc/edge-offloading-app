...
Java_org_portablecl_poclaisademo_JNIPoclImageProcessor_initPoclImageProcessor(JNIEnv *env,
    ...
    out_mask_buf = clCreateBuffer(context, CL_MEM_READ_WRITE, tot_pixels * MAX_DETECTIONS * sizeof(cl_char) / 4,
                                         NULL, &status);
    CHECK_AND_RETURN(status, "failed to create the segmentation mask buffer");

    // RGBA:
    postprocess_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY, tot_pixels * 4 * sizeof(cl_char) / 4,
                                         NULL, &status);
    CHECK_AND_RETURN(status, "failed to create the segmentation mask buffer");
    ...
    int arg_idx = 0;
    status = clSetKernelArg(dnn_kernel, arg_idx++, sizeof(cl_mem), &img_buf);
    status |= clSetKernelArg(dnn_kernel, arg_idx++, sizeof(cl_int), &inp_w);
    status |= clSetKernelArg(dnn_kernel, arg_idx++, sizeof(cl_int), &inp_h);
    status |=
            clSetKernelArg(dnn_kernel, arg_idx++, sizeof(cl_int), &rotate_cw_degrees);
    status |= clSetKernelArg(dnn_kernel, arg_idx++, sizeof(cl_int), &inp_format);
    status |= clSetKernelArg(dnn_kernel, arg_idx++, sizeof(cl_mem), &out_buf);
    status |= clSetKernelArg(dnn_kernel, arg_idx++, sizeof(cl_mem), &out_mask_buf);

    arg_idx = 0;
    status = clSetKernelArg(postprocess_kernel, arg_idx++, sizeof(cl_mem), &out_buf);
    status |= clSetKernelArg(postprocess_kernel, arg_idx++, sizeof(cl_mem), &out_mask_buf);
    status |= clSetKernelArg(postprocess_kernel, arg_idx++, sizeof(cl_mem), &postprocess_buf);
...
Java_org_portablecl_poclaisademo_JNIPoclImageProcessor_destroyPoclImageProcessor(JNIEnv *env,
    ...
    if (nullptr != postprocess_buf){
        clReleaseMemObject(postprocess_buf);
        postprocess_buf = nullptr;
    }
...
Java_org_portablecl_poclaisademo_JNIPoclImageProcessor_poclProcessYUVImage(JNIEnv *env,
    ...
    if (do_segment) {
        ...
        status = clEnqueueReadBuffer(commandQueue[device_index], postprocess_buf, CL_FALSE, 0,
