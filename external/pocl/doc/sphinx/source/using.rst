...
It is possible to profile pocl and pocld using Tracy profiler: https://github.com/wolfpld/tracy.
Tracy allows both instrumentation and sampling profiling.

To use Tracy, recompile pocl/pocld with `TRACY_ENABLE` CMake option enabled. To
enable sampling profiling, disable the `TRACY_NO_SAMPLING` CMake option. Support
for more Tracy options can be added if necessary.

Note that the Tracy client is built as a dynamic library to allow profiling
projects consisting of multiple dynamic libraries, such as those using builtin
kernels. As such, the `TRACY_STATIC` option should be turned off.

If you use pocl in your application and want to debug both your application and
pocl with Tracy, add pocl's Tracy subdirectory as a CMake subdirectory in your
project and turn on `TRACY_ENABLE` in both your application and pocl.

Currently, the level instrumentation is rather sparse, and Tracy is not
propagated everywhere. For example, when pocld is profiled, the traces from pocl
library are missing.

To view Tracy traces, you'll need to build the profiler GUI. It can be found in
`tools/tracy/profiler`. For requirements, building instructions, and basic usage
consult the user manual.

**Future work:**

- Allow disabling/enabling Tracy with environment variable (i.e., build with
  TRACY_ENABLE=ON, turn off at runtime).
- Add more thorough instrumentation.
- Solve missing traces e.g. from libpocl when profiling pocld (likely due to not
  linking Tracy or not propagating TRACY_ENABLE correctly)
